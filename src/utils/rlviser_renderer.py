...
class RLViserRenderer(Renderer[GameState]):
    ...
    def __init__(self, tick_rate=120/8):
        # Lancer le serveur RLViser
        # Note: rlviser.exe doit être dans le répertoire courant ou dans le PATH
        import os

        # Vérifier si rlviser.exe existe
        if not os.path.exists("./rlviser.exe") and not os.path.exists("rlviser.exe"):
            print("Warning: rlviser.exe non trouvé dans le répertoire courant")
            print("Téléchargez-le depuis: https://github.com/VirxEC/rlviser/releases")

        print("Lancement de RLViser...")
        rlviser.launch()

        rlviser.set_boost_pad_locations(BOOST_LOCATIONS)
        self.tick_rate = tick_rate
        self.packet_id = 0
        print("RLViser server started on http://localhost:8080")
