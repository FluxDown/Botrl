"""
Script d'entraînement amélioré avec callbacks TensorBoard enrichis et évaluation périodique
"""
...
def create_environment(config, eval_mode=False):
...
def train():
    """Fonction principale d'entraînement avec callbacks améliorés"""
    ...
    logger.log_text("=" * 50)
    logger.log_text("Starting Rocket League Bot Training (Improved)")
    logger.log_text("=" * 50)
    ...
    while total_timesteps < training_config['total_timesteps']:
        ...
        while not done and rollout_buffer.ptr < training_config['batch_size']:
            ...
            # Callback on_step (pour logger les infos du step)
            # Note: info peut contenir reward_parts si disponible
            ...
            if done:
                ...
                # Ajouter les stats d'épisode dans info pour le callback
                episode_info = {
                    'episode': {'r': episode_reward, 'l': episode_length}
                }
                # Marquer si c'est un succès (vous pouvez définir votre propre critère)
                # Par exemple: but marqué si reward > seuil
                if episode_reward > 50:  # Ajustez selon vos critères
                    episode_info['success'] = 1.0
                else:
                    episode_info['success'] = 0.0
        ...
        if total_timesteps % training_config['save_interval'] == 0:
            ...
            tb_callback.on_checkpoint_save(checkpoint_path, total_timesteps, episode_reward)
